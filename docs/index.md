...
description: Set up your API key and integrate the SmartSpectra SDK in your app.
---

## 1. Get an API key

Register at [physiology.presagetech.com](https://physiology.presagetech.com/auth/register) to get a free API key. See [Authentication](authentication.md) for OAuth setup and key management.
...
- [Run headless](headless-mode.md) — skip the built-in UI and handle camera + results yourself
- [Data types reference](data-types.md) — protobuf message structures returned by the SDK
...
The SDK exposes one language-agnostic metrics payload across platforms. When platform pages lag the underlying SDK, this overview uses the highest currently documented capability and calls out validation gaps directly. See the [payload schema](data-types.md) for field names and wire types.
