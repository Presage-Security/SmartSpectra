---
title: SmartSpectra Metrics Payload Data Types
description: The language-agnostic SmartSpectra metrics payload — every message, enum, and field the SDK emits.
...
- [`InsightType`](#insighttype) `type` -- Origin of this insight. Always INSIGHT_TYPE_VITALS today — SPEECH and COMBINED are reserved and never set — so route on request_id rather than on this field to tell auto-fired snapshots from on-demand replies.
